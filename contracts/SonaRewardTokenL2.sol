...
import { SonaMinterL2 as SonaMinter } from "./access/SonaMinterL2.sol";
...
import { ERC721Upgradeable as ERC721, IERC721Upgradeable as IERC721 } from "openzeppelin-upgradeable/token/ERC721/ERC721Upgradeable.sol";
...
import { IOptimismMintableERC721 } from "./interfaces/IOptimismMintableERC721Upgradeable.sol";
	...
	IOptimismMintableERC721
{
	...
	function setRemoteTokenAddress(
		address _newToken
	) external onlySonaAdmin reinitializer(2) {
	...
	function safeMint(address _owner, uint256 _tokenId) external onlySonaMinter {
		_mint(_owner, _tokenId);
	}

	function burn(address, uint256 _tokenId) external onlySonaMinter {
		_burn(_tokenId);
	}
