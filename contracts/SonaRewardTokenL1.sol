...
contract SonaRewardTokenL1 is SonaMinter, ISonaRewardToken, IERC2981 {
	...
	function migratetoL2(
		...
		IERC721Bridge _bridge
	) external {
		isApprovedForAll[address(this)][address(_bridge)] = true;

		for (uint256 i = 0; i < _tokenIds.length; i++) {
			uint256 tokenId = _tokenIds[i];
			if (ownerOf(tokenId) != msg.sender) revert SonaRewardToken_Unauthorized();
			transferFrom(msg.sender, address(this), tokenId);

			_bridge.bridgeERC721To(
				address(this),
				address(this),
				msg.sender,
				tokenId,
		...
		isApprovedForAll[address(this)][address(_bridge)] = false;
	}
